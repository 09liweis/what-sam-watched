...
<template>
  <NuxtLayout>
    <main class="conatiner p-5">
      ...
      <button @click="searchAndUpsert()">{{loading?'Loading':'Add'}}</button>
      <article v-for="v in visuals" :key="v.id" class="flex justify-between mb-1.5">
        ...
        <div class="flex">
          <button class="mr-2 shadow bg-indigo-500 text-white p-1 flex rounded-md" @click="updateVisual(v)">
            <svg v-if="v.loading" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
